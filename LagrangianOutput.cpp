...
LagrangianOutput::LagrangianOutput(MPI_Comm &comm_, LagrangianMeshOutputData &iod_lag_)
                : iod_lag(iod_lag_), comm(comm_), disp_file(NULL), force_file(NULL),
                  fovera_file(NULL), surfp_file(NULL)
{
  iFrame = 0;
...
void
LagrangianOutput::OutputResults(double t, double dt, int time_step, 
...
*/
  ...
  if(strcmp(iod_lag.surface_pressure,"")) {
    char outname[512];
    sprintf(outname, "%s%s", iod_lag.prefix, iod_lag.surface_pressure);

    if(surfp_file == NULL) { //create new file and write header
      surfp_file = fopen(outname, "w");
      if(surfp_file == NULL) {//unable to open file
        fprintf(stdout,"\033[0;31m*** Error: Cannot write file %s.\n\033[0m", outname);
        exit(-1);
      }
      fprintf(surfp_file, "Scalar SURFACE_PRESSURE under NLDynamic for MyNodes\n");
      fprintf(surfp_file, "%d\n", (int)P.size());
      fclose(surfp_file);
    }

    surfp_file = fopen(outname,"a");
    if(surfp_file == NULL) {//unable to open file
      fprintf(stdout,"\033[0;31m*** Error: Cannot write file %s.\n\033[0m", outname);
      exit(-1);
    }
    AppendResultToFile(surfp_file, t, P.size(), 1, P.data());
  }
